...
static void efficient_heapify(PriorityQueue pqueue, Vector values) {
	// Προσθηκη στοιχειων απο το vector χωρις να μας ενδιαφερει η σειρα
	int size = vector_size(values);
	for (int i=0; i<size; i++) {
		vector_insert_last(pqueue->vector, vector_get_at(values, i));	
	}
	// Κανουμε bubble_down ωστε να φερουμε το μεγαλυτερο στοιχειο στην κορυφη του δεντρου
	// Με αυτον τον τροπο καθε υποδεντρο θα ειναι ενας σωρος
