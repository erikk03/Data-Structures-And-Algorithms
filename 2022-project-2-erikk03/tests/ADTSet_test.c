...
// Τεστ για την υλοποιηση ADTSet 
// Με την χρηση ομως της λειτουργιας set_create_from_sorted_values
//
void test_create2(void) {
	
	// Δημιουργια ενος Vector
	Vector values = vector_create(0, free);
	
	// Προσθηκη στοιχειων στο Vector
	for(int i=0; i<=11; i++){
		vector_insert_last(values, create_int(i));
	}
	// Δημιουργούμε ένα set (χωρίς συνάρτηση αποδεύσμευσης)
	Set set = set_create_from_sorted_values(compare_ints, NULL, values);
	set_set_destroy_value(set, NULL);

	TEST_ASSERT(set != NULL);
	TEST_ASSERT(set_size(set) == vector_size(values));
...
void test_remove2(void) {
	...
	int prev_size = set_size(set2);					// size of set after the insert of local_value1
	TEST_ASSERT(set_remove(set2, &local_value1));
	TEST_ASSERT(set_is_proper(set2));
	TEST_ASSERT(set_size(set2) == prev_size-1);		// size of set must be one less than prev_size
...
void test_node_value2(void) {
	...
	// Εισάγουμε έναν αριθμό και αποθηκεύουμε το node
	set_insert(set, value_array[0]);
	SetNode node = set_find_node(set, value_array[0]);
	TEST_ASSERT(set_node_value(set, node) == value_array[0]);
...
TEST_LIST = {
	...
	{ "set_create_from_sorted_values", test_create2},
	{ "set_insert_from_sorted_values", test_insert2},
	{ "set_remove_from_sorted_values", test_remove2},
	{ "set_find_from_sorted_values", test_find2},
	{ "set_iterate_from_sorted_values", test_iterate2},
	{ "set_node_value_from_sorted_value", test_node_value2},
