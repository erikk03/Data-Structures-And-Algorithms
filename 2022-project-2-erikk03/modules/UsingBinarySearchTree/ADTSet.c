...
struct set {
	...
	int set_steps;				// Βηματα που εκτελει η καθε λειτουργια
...
// Συναρτηση η οποια κατασκευαζει με αναδρομικο τροπο ενα BST απο ενα ταξινομημενο Vector
static SetNode Vector_to_BST(Vector values, int start, int end){
	if(start > end){
		return NULL;
	}
	int middle = (start+end)/2;									// Το μεσον του Vector
	SetNode node = node_create(vector_get_at(values, middle)); 	// Δημιουργια ενος node που θα εχει ως node->value την τιμη του Vector στη θεση middle
	
	node->left = Vector_to_BST(values, start, middle-1);		// Δημιουργια αριστερου υποδεντρου αναδρομικα

	node->right = Vector_to_BST(values, middle+1, end);			// Δημιουργια δεξιου υποδεντρου αναδρομικα
	
	
	return node;												// Επιστρεφουμε τη ριζα
}
...
Set set_create_from_sorted_values(CompareFunc compare, DestroyFunc destroy_value, Vector values){
	...
	Set set = malloc(sizeof(*set));
	set->root = Vector_to_BST(values, 0, vector_size(values)-1);
	set->size = vector_size(values);
	set->set_steps = vector_size(values);
	set->compare = compare;
	set->destroy_value = destroy_value;
